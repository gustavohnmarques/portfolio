...
const ModalProjectDetails: React.FC<ModalProjectDetailsProps> = ({
  onRequestClose,
  title,
  detailDescription,
  developedFor,
  ...
  const renderGallery = useMemo(() => {
    return gallery.map((item, index) => (
      <SwiperSlide key={index}>
        {item.type === "image" ? (
          <img
            src={import.meta.env.VITE_BUCKET_URL + item.src}
            style={{
              width: "100%",
              borderRadius: "8px",
              height: "350px",
  ...
  const renderIdicators = useMemo(() => {
    return gallery.map((item, index) => (
      ...
      >
        {item.type == "image" ? (
          <img src={import.meta.env.VITE_BUCKET_URL + item.src} />
        ) : (
    ...
    >
      <S.ModalContent ref={modalContentRef}>
        <S.ModalHeader>
          ...
          <i className="fab fa-x" onClick={onRequestClose}>
            {" "}
          </i>
        </S.ModalHeader>
        <S.ModalBody>
          <S.GalleryContainer>
            ...
            <S.SwiperIndicators>{renderIdicators}</S.SwiperIndicators>
          </S.GalleryContainer>
